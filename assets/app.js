...
function renderAssets(){ const c = document.getElementById('assetsTable'); if(!c) return; const rows = state.assets.map(a=>`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px dashed rgba(255,255,255,0.02)"><div style="flex:1"><div><strong>${a.name}</strong> <span class="muted-sm">${a.category}</span></div></div><div style="width:140px;text-align:right">${fmtNumber(a.value)}</div><div style="width:110px;text-align:right"><button class="btn secondary" data-act="edit-asset" data-id="${a.id}">Edit</button><button class="btn" data-act="del-asset" data-id="${a.id}">Delete</button></div></div>`).join(''); c.innerHTML = rows || '<div class="muted-sm">No assets added</div>'; }
...
function renderLiabilities(){ const c = document.getElementById('liabilitiesTable'); if(!c) return; const rows = state.liabilities.map(l=>`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px dashed rgba(255,255,255,0.02)"><div style="flex:1"><div><strong>${l.name}</strong> <span class="muted-sm">${l.category}</span></div><div class="muted-sm">Due ${l.dueDate||'—'}</div></div><div style="width:140px;text-align:right">${fmtNumber(l.amount)}</div><div style="width:110px;text-align:right"><button class="btn secondary" data-act="edit-liability" data-id="${l.id}">Edit</button><button class="btn" data-act="del-liability" data-id="${l.id}">Delete</button></div></div>`).join(''); c.innerHTML = rows || '<div class="muted-sm">No liabilities</div>'; }
...
function drawNetworthChart(){ const canvas = document.getElementById('networthChart'); if(!canvas) return; const ctx = canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height); const now = new Date(); const months = []; for(let i=11;i>=0;i--){ const d = new Date(now.getFullYear(), now.getMonth()-i,1); months.push({label: d.toLocaleString(undefined,{month:'short'}), year:d.getFullYear(), month:d.getMonth()}); }
  ...
  const min = Math.min(...points) * 1.05; const max = Math.max(...points) * 1.05; const w = canvas.width; const h = canvas.height; ctx.strokeStyle = 'rgba(255,255,255,0.06)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(30,h-30); ctx.lineTo(w-10,h-30); ctx.stroke(); ctx.beginPath(); ctx.moveTo(30,10); ctx.lineTo(30,h-30); ctx.stroke(); ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.font='11px Inter, system-ui'; months.forEach((m,i)=>{ const x = 30 + (i/(months.length-1))*(w-50); ctx.fillText(m.label, x-10, h-10); }); ctx.beginPath(); points.forEach((p,i)=>{ const x = 30 + (i/(points.length-1))*(w-50); const y = h-30 - ((p - min) / (max-min || 1))*(h-50); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }); ctx.strokeStyle = 'rgba(79,70,229,0.95)'; ctx.lineWidth=2; ctx.stroke(); points.forEach((p,i)=>{ const x = 30 + (i/(points.length-1))*(w-50); const y = h-30 - ((p - min) / (max-min || 1))*(h-50); ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fillStyle='white'; ctx.fill(); }); }

function drawExpensePie(){ const canvas = document.getElementById('expensePie'); if(!canvas) return; const ctx = canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height); const w = canvas.width; const h = canvas.height; const r = Math.min(w,h)/2 - 10; const cx = w/2; const cy = h/2; const expenses = state.transactions.filter(t=>t.type==='expense'); const categorySums = {}; expenses.forEach(e=>{ categorySums[e.category] = (categorySums[e.category]||0) + Number(e.amount||0); }); const entries = Object.entries(categorySums); if(entries.length===0){ ctx.fillStyle='rgba(255,255,255,0.04)'; ctx.font='13px Inter'; ctx.fillText('No expenses yet', cx-40, cy); return; } const total = entries.reduce((s,[k,v])=>s+v,0); let start = -Math.PI/2; const colors = ['#7c3aed','#06b6d4','#f97316','#10b981','#ef4444','#eab308','#3b82f6']; entries.forEach(([k,v],i)=>{ const slice = v/total * Math.PI*2; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,r,start,start+slice); ctx.closePath(); ctx.fillStyle = colors[i%colors.length]; ctx.fill(); const mid = start + slice/2; const lx = cx + Math.cos(mid)*(r+18); const ly = cy + Math.sin(mid)*(r+18); ctx.fillStyle='rgba(255,255,255,0.9)'; ctx.font='11px Inter'; ctx.fillText(k + ' ' + Math.round(v/total*100) + '%', lx-20, ly); start += slice; }); }
...
function initUI(){
  ...
  const assetForm = document.getElementById('assetForm'); if(assetForm) assetForm.addEventListener('submit', e=>{ e.preventDefault(); const id = document.getElementById('assetId').value; const obj = { id: id||uid(), name: document.getElementById('assetName').value, category: document.getElementById('assetCategory').value, value: Number(document.getElementById('assetValue').value||0) }; if(id){ const ix = state.assets.findIndex(a=>a.id===id); if(ix>=0) state.assets[ix]=obj; } else state.assets.push(obj); debouncedSave(); const wrap = document.getElementById('assetFormWrap'); if(wrap) wrap.style.display='none'; });
  const assetsTable = document.getElementById('assetsTable'); if(assetsTable) assetsTable.addEventListener('click', e=>{ const act = e.target.dataset.act; const id = e.target.dataset.id; if(!act) return; if(act==='edit-asset'){ const a = state.assets.find(x=>x.id===id); if(!a) return; document.getElementById('assetId').value=a.id; document.getElementById('assetName').value=a.name; document.getElementById('assetCategory').value=a.category; document.getElementById('assetValue').value=a.value; const wrap = document.getElementById('assetFormWrap'); if(wrap) wrap.style.display='block'; const title = document.getElementById('assetFormTitle'); if(title) title.textContent='Edit Asset'; } if(act==='del-asset'){ if(confirm('Delete asset?')){ state.assets = state.assets.filter(x=>x.id!==id); debouncedSave(); } } });
  ...
  const liabilityForm = document.getElementById('liabilityForm'); if(liabilityForm) liabilityForm.addEventListener('submit', e=>{ e.preventDefault(); const id = document.getElementById('liabilityId').value; const obj = { id: id||uid(), name: document.getElementById('liabilityName').value, category: document.getElementById('liabilityCategory').value, amount: Number(document.getElementById('liabilityAmount').value||0), dueDate: document.getElementById('liabilityDue').value || null }; if(id){ const ix = state.liabilities.findIndex(c=>c.id===id); if(ix>=0) state.liabilities[ix]=obj; } else state.liabilities.push(obj); debouncedSave(); const wrap = document.getElementById('liabilityFormWrap'); if(wrap) wrap.style.display='none'; });
  const liabilitiesTable = document.getElementById('liabilitiesTable'); if(liabilitiesTable) liabilitiesTable.addEventListener('click', e=>{ const act = e.target.dataset.act; const id = e.target.dataset.id; if(!act) return; if(act==='edit-liability'){ const a = state.liabilities.find(x=>x.id===id); if(!a) return; document.getElementById('liabilityId').value=a.id; document.getElementById('liabilityName').value=a.name; document.getElementById('liabilityCategory').value=a.category; document.getElementById('liabilityAmount').value=a.amount; document.getElementById('liabilityDue').value=a.dueDate||''; const wrap = document.getElementById('liabilityFormWrap'); if(wrap) wrap.style.display='block'; const title = document.getElementById('liabilityFormTitle'); if(title) title.textContent='Edit Liability'; } if(act==='del-liability'){ if(confirm('Delete liability?')){ state.liabilities = state.liabilities.filter(x=>x.id!==id); debouncedSave(); } } });
